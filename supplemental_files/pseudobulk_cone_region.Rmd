...
#### All genes
```{r}
for (i in region_table %>% filter(padj < 0.05) %>% pull(Ensembl)){
  #print(i)
  gene <- region_table %>% filter(Ensembl == i) %>% pull(Gene)
  print(VlnPlot(scEiaD__subset, i, split.by = 'retina_region', group.by='study_accession', log = TRUE)  + ggtitle(gene))
}
```
