...
spec_diff <- diff_tab_big %>% 
  ...
  summarise(Count = n()) %>% 
  filter(Count > 1) %>% summarise(Count = n()) %>% filter(Count < 3) %>% mutate(go = paste(Symbol, '___', Organism, sep = '')) %>% pull(go)

diff_tab_big %>% 
  filter(Symbol %in% diff_tab$Symbol) %>% # only retain diff genes across "All"
  group_by(Symbol, Organism, Base) %>% 
  summarise(Count = n()) %>% 
  filter(Count > 1) %>% mutate(go = paste(Symbol, '___', Organism, sep = '')) %>% 
  filter(go %in% (spec_diff))  %>% 
  select(-go) %>% 
  ungroup() %>% 
