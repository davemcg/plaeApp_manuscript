...
gene_sets <- diff_tab %>% 
  ...
  slice_max(n=20, order_by = log2FoldChange)
  
g <- gene_sets %>% filter(Organism == 'Homo sapiens') %>%  mutate(gk = paste0(Symbol, ' (', ID, ')')) %>% pull(gk) %>% unique()

x <- scEiaD_2020_v01 %>% tbl("diff_testing") %>% filter(Gene %in% g) %>% collect() %>% filter(Group == 'CellType (Predict)', Organism == 'Homo sapiens', Against == 'All') %>% select(Gene, log2FoldChange, Base) %>% pivot_wider(values_from=log2FoldChange, names_from = Base) 
