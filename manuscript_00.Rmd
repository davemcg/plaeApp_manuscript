...
tab_cap <- tab_cap(name = 'web_resources', caption = 'Comparison of PLAE with other web resources that contain single cell ocular datasets, as of 2021-03-15.')
...
## Differential testing identifies thousands genes enriched across various ocular cell types

We used a "pseudo bulk" based approach wherein the counts for a category (CellType, Cluster, or CellType (Predict)) were summed for each category - organism - study grouping. The creates a counts matrix which has statistical properties matching that of "bulk" RNA seq experiment matrices. This approach has been found to be more specific in identifying differentially expressed genes in single cell RNA-seq experiments [@squairConfrontingFalseDiscoveries2021]. Once the aggregated matrix was constructed, we use the DESeq2 differential gene expression analysis software to identify differentially expressed genes. We tested each species (Human, Macaque, Mouse) separately. We had an insufficient number of chicken studies to conduct differential testing. The model used for DESeq2 was "~study_accession + category" in order to regress out study specific differential expression. After running these tests we extracted differentially expressed genes with two types of contrasts: either against all remaining categories (e.g. cones versus all remaining cell types) or in a pair-wise manner (cone against rod). We fin
```{r}
# fix  occasional NA
...
diff_tab <- diff_tab_big %>% 
  filter(Against == 'All', Base %in% (meta_filter %>% filter(Organ == 'Eye') %>% pull(CellType) %>% unique())) 
...
Many of these genes can be differentially expressed in more than one cell type. To identify genes that are consistently differentially expressed we can use a more specific test. We define a consistent gene that is 

```{r}
spec_diff <- diff_tab_big %>% 
  filter(Symbol %in% diff_tab$Symbol) %>% # only retain diff genes across "All"
  group_by(Symbol, Organism, Base) %>% 
  summarise(Count = n()) %>% 
  summarise(Count = n())



spec_diff_go <- spec_diff %>% 
  filter(Count <= 3) %>% 
  mutate(go = paste(Symbol, '___', Organism, sep = '')) %>% 
  pull(go)
...
diff_tab_big %>% 
  ...
  #mutate(go = paste(Symbol, '___', Organism, sep = '')) %>% 
  #filter(go %in% (spec_diff_go))  %>% 
  #select(-go) %>% 
  ungroup() %>% 
  group_by(Organism, Base) %>% 
...
g <- gene_sets %>% filter(Organism == 'Homo sapiens') %>%  mutate(gk = paste0(Symbol, ' (', ID, ')')) %>% pull(gk) %>% unique()

# only uses CT that exist in the organism <-> CellType 
ct <- meta_filter %>% filter(organism == 'Homo sapiens') %>% pull(CellType) %>% unique()

x <- scEiaD_2020_v01 %>% tbl("diff_testing") %>% filter(Gene %in% g, Base %in% ct) %>% collect() %>% filter(Group == 'CellType (Predict)', Organism == 'Homo sapiens', Against == 'All') %>% select(Gene, log2FoldChange, Base) %>% pivot_wider(values_from=log2FoldChange, names_from = Base) 
...
g <- gene_sets %>% filter(Organism == 'Mus musculus') %>%  mutate(gk = paste0(Symbol, ' (', ID, ')')) %>% pull(gk) %>% unique()

x <- scEiaD_2020_v01 %>% tbl("diff_testing") %>% filter(Gene %in% g) %>% collect() %>% filter(Group == 'CellType (Predict)', Organism == 'Mus musculus', Against == 'All') %>% select(Gene, log2FoldChange, Base) %>% pivot_wider(values_from=log2FoldChange, names_from = Base) 
celltype <- colnames(x[,-1])
x <- x %>% data.frame()
row.names(x) <- x$Gene
x <- x[,-1]
...
```{r}
# top 100 genes in each organism/base category
diff_tab %>% 
  
  group_by(Base, Organism) %>% 
  top_n(., 100, -padj) %>% 
  arrange(Base, Organism) %>% 
  relocate(Base, Organism) %>% 
  select(-Against, -Group)
```

```{r}
# consistently diff genes
consistent <- diff_tab %>% 
  group_by(Symbol, Base)  %>% 
  summarise(Count = n()) %>% 
  filter(Count > 1) %>% 
  mutate(k = paste(Symbol, Base, sep = '_'))

diff_tab %>% mutate(k = paste(Symbol, Base, sep = '_')) %>% 
  filter(k %in% consistent$k) %>% 
  group_by(Base, Organism) %>% 
  summarise(Count = n()) %>% 
  rename(`CellType (Predict)` = Base, `Differentially Expressed Gene Count` = Count) %>% 
  pivot_wider(values_from = `Differentially Expressed Gene Count`, names_from = Organism) %>% replace(is.na(.),0) %>% 
...
down_tab <- scEiaD_2020_v01 %>% tbl('diff_testing') %>% 
  filter(Group == 'CellType (Predict)', padj < 1e-5, (log2FoldChange) < -2) %>% 
  collect() %>% 
  filter(Against == 'All', Base %in% (meta_filter %>% filter(Organ == 'Eye') %>% pull(CellType_predict) %>% unique())) %>% mutate(k = paste(Symbol, Base, sep = '_'))

diff_tab %>% mutate(k = paste(Symbol, Base, sep = '_')) %>% 
  filter(k %in% down_tab$k, Base %in% c('AC/HC Precursor', 'Amacrine Cell', 'Astrocyte', 'Bipolar Cell', 'Cone', 'Early RPC',' Horizontal Cell','Late RPC','Microglia','Muller Glia', 'Neurogenic Cell', 'Photoreceptor Precursor','Retinal Ganglion Cell','Rod','Rod Bipolar Cell','RPC','RPE' )) %>% 
  arrange(padj)

diff_tab_big %>% 
  filter(Symbol %in% diff_tab$Symbol) %>% # only retain diff genes across "All"
  group_by(Symbol, Base, Organism) %>% 
  summarise(Count = n()) %>% 
  filter(Count > 8, Base == 'Rod') %>% arrange(-Count)
```

```{r diff_difff_exp_genes, fig.height = 3, fig.width = 8, echo = F, message = F}
source('src/make_exp_plot.R')

input <- list()
input$exp_plot_groups <- c('organism')
input$exp_plot_facet <- c('CellType_predict')
input$exp_plot_genes <- c('SCRG1 (ENSG00000164106)')
