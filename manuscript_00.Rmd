...
Our full scEiaD pipeline was published previously in Swamy et al. [@swamyBuildingMegaSinglecell2021]. It is composed of three Snakefiles which perform three core workflow tasks[@molderSustainableDataAnalysis2021]. The "SnakeQUANT" pipeline takes the raw fastq input and produces a gene by cell count matrix with kallisto and bustools [@melstedBarcodeUMISet2019; @melstedModularEfficientPreprocessing2019]. This matrix is used in the "SnakePOP" pipeline to identify the optimal parameters for the scANVI based batch correction process (`r supFig_cap(name = 'scPOP_performance', display = 'cite')`). Outputs from this pipeline include the batch corrected lower dimensional space, the UMAP coordinates, cell cluster assignments, and the cell type predictions. The outputs from this pipeline are used in the "SnakeSCEIAD" which performs differential gene testing across multiple conditions and wraps the various data types into the scEiaD sqlite database for use in the plae app (`r supFig_cap(name = 'workflow_network', display = 'cite')`). 

## Quality Control 

To decide whether a droplet is considered a viable cell, we use the "barcodeRanks" algorithm from DropletUtils to automatically identify empty droplets. We then kept cells with the following criteria: less than 20% of the counts were in mitochondrial genes, a cell had more than 600 total counts, at least 300 unique genes quantified, and fewer than 100,000 total counts. To identify likely doublets we used Scrublet [@wolockScrubletComputationalIdentification2019a] and doubletCells (from scran) [@lunStepbystepWorkflowLowlevel2016a] to generate per cell doublet probability scores. To identify cells for removal we used the scores in two ways. First, if an individual cell had a Scrublet score greater than 0.8 (where 1.0 is 100% probability of doublet) and a doubletCell score greater than 1,000,000 (unitless measure where higher value is more likely to be doublet) the cell is labelled as a doublet. Second, if a cluster has an average Scrublet score greater than 0.2 or an average doubletCell score greater than 12 the entire cluster was marked as doublets. These cutoffs were selected by hand inspecting density distributions of the Scrublet and doubletCell scores.

## Alterations from Swamy et al. to increase stringency of the dataset

The following alterations were made to Swamy et al.: First, the cutoff to retain a cell was raised from 200 quantified unique genes to 300. Second, we use the DecontX algorithm[@yangDecontaminationAmbientRNA2020] from the celda R package (version 1.9.2) to automatically remove ambient RNA contamination on a per-study basis to produce a updated count matrix. Third, we altered the procedure for aligning gene names across species (see below for details). Fourth, we use the scANVI (scvitools based) batch correction method, which leverages information from the known cell type labels in the batch correction process [@xuProbabilisticHarmonizationAnnotation2021].
...
We found in Swamy et al. that scVI had the best batch integration performance relative to other tools like Seurat's CCA, fastMNN, etc. For this study we chose to use a variant of scVI, scANVI, We also found that adjusting the number of latent dimensions, highly variable genes, and epochs had substantial effects on the integration performance. For this update of scEiaD we ran integration with 3000, 4000, 5000, and 6000 highly variable genes, 5 and 15 epochs, and 6, 8, 10, 15, and 20 scANVI outputted latent dimensions. After evaluating the performance of the new scEiaD dataset with our scPOP package we selected the following parameters for the integration with scANVI: 15 latent dimensions, 4000 highly variable genes, and 5 epochs [@swamyBuildingMegaSinglecell2021]. The UMAP was built with a min_dist of 0.1 and the clustering used 50 nearest neighbors. 
...
web_resources_cap <- tab_cap(name = 'web_resources', caption = 'Comparison of PLAE with other web resources that contain single cell ocular datasets, as of 2021-03-15. The Protein Atlas is asterisked as while it contains independent datasets, it only has one ocular study. Export-able means whether a resource allows bulk downloads of the count matrices and metadata. Organisms refers to the number of ocular species used.')
