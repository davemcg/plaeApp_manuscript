...
consist_diff3 <- scEiaD_2020_v01 %>%
  ...
  filter(`mean log2FC` > 4, `sig count` > 1, `mean padj` < 1e-5) %>%
  collect() %>%
...
abc <- consist_diff3 %>% filter(Symbol %in% uniq_g$Symbol) %>% group_by(Base) %>% summarise(Count = n()) %>% ggplot(aes(x=`Base`, y = `Count` )) + geom_bar(stat='identity', position = 'dodge') + cowplot::theme_cowplot() + coord_flip() + xlab("CellType (Predict)")
#toc()
```
