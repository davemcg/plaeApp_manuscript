...
diff_tab_sup %>% pivot_longer(-`CellType (Predict)`, names_to = 'Organism', values_to = 'Diff Gene Count') %>% ggplot(aes(x=`CellType (Predict)`, y = `Diff Gene Count`, fill = Organism )) + geom_bar(stat='identity', position = 'dodge') + cowplot::theme_cowplot() + coord_flip()
```
