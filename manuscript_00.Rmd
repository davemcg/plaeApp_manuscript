...
p_data <- meta_filter %>% 
  ...
  #                                    TRUE ~ CellType_predict)) %>% 
  filter(Organ == 'Eye') %>% 
  mutate(Citation = paste(str_trunc(Citation, width = 20), str_trunc(batch, side = "left", width = 15))) %>% 
...
plot_grid(a_aa_b, c_d, proportion_bar_plot, ncol = 3, rel_widths = c(1,1.6, 3))
```
...
diff_tab_sup <- diff_tab %>% 
  ...
  as_tibble()
write_csv(diff_tab_sup, file = 'supplemental_files/differential_expression_table.csv')
diff_tab_sup %>% 
  flextable()
...
## Well supported ocular cell types identify 
...
g <- consist_diff %>% filter(Symbol %in% uniq_g$Symbol) %>% group_by(Base) %>% slice_max(n=5, order_by = `mean log2FC`) %>% mutate(g = paste0(Symbol, ' (', ID, ')')) %>% pull(g)
...
hm <- ComplexHeatmap::Heatmap(x, column_labels = organism, 
                              ...
                              row_title_rot = 0)
ComplexHeatmap::draw(hm, padding = unit(c(2, 2, 30, 2), "mm"))
```
...
x <- scEiaD_2020_v01 %>% 
  tbl("diff_testing") %>% 
  filter(Base %in% c("Early RPC", "Late RPC", "RPC")) %>%  
  filter(Group == 'CellType (Predict)', 
         Against %in% c("Early RPC", "Late RPC", "RPC")) %>% 
  group_by(Base, Against, Gene) %>% 
  summarise(`mean log2FC` = mean(log2FoldChange), `mean padj` = mean(padj), `sig count` = sum(pvalue < 1e-3)) %>% 
  arrange(-`mean log2FC`) %>%   
  collect() %>% 
...
down_tab <- scEiaD_2020_v01 %>% tbl('diff_testing') %>% 
  ...
  collect() %>% 
  filter(Against == 'All', Base %in% (meta_filter %>% filter(Organ == 'Eye') %>% pull(CellType_predict) %>% unique())) %>% 
  mutate(k = paste(Symbol, Base, sep = '_'))
