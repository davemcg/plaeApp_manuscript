...
## New studies identified since 2019 data run

## scEiaD re-created and pipeline updated

The scEiaD pipeline is largely the same as in Swamy et al. [@swamy_building_2021]. Briefly, we collect the raw fastq files and quantify with the kallisto with the bustools system [@melsted_barcode_2019; @melsted_modular_2019]. The quantified counts are merged into one large matrix that is used for downstream processing with scvitools to batch correct the data and Seurat for quality control, cluster finding, UMAP finding. We use a xgboost-based machine learning approach to transfer cell type labels to unlabelled cells.  

The following alterations were made: First, the cutoff to retain a cell was raised from 200 unique genes quantified to 300 Second, we use the DecontX algorithm [@yang_decontamination_2020] from the celda R package (version 1.9.2) to automatically remove ambient RNA contamination. Third, we altered the procedure for automatically aligning gene names across species (see below for details). Fourth, we use the scANVI (scvitools based) batch correction method, which leverages the known cell type labels in batch correction. 

## Cross species gene alignment

As we have four species in scEiaD (*Gallus gallus*, *Macaca fascicularis*, *Mus musculus*, and *Homo sapiens*) that we unify in a combined data analysis, we must align shared genes. We use a multi-step procedure. 
...
While PLAE and Spectacle both contain large numbers of ocular-related scRNA datasets, they are structured differently. PLAE is built around the scEiaD database, which runs an extensive multi-stage pipeline to build and identify a high performing scVI-based eye model that is used to integrate all datasets together. This allows for queries to be constructed across studies and 

The Spectacle web app contains access 87,305 single cell transcriptomes across four publications.
