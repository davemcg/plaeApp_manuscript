...
p_data %>% ggplot(aes(y=Citation,x=Proportion,fill=CellType_predict)) + geom_bar(stat= 'identity') +
...
ws_oct <- meta_filter %>%  
  ...
  summarise(Count = n()) %>% 
  filter(Count > 1, !is.na(CellType_predict)) %>% 
...
base_yo <- consist_diff %>% filter(Symbol %in% genes) %>% pull(Base)
