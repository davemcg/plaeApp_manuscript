...
```{r, fig.height=14, fig.width=12, fig.cap = scEiaD_data_overview_cap, echo = FALSE}
...
timing_plot <- timings %>% as_tibble(rownames = 'Test') %>% 
  ...
  mutate(Text = case_when(is.na(value) ~ 'Not\npossible',
                          TRUE ~ '')) %>% 
...
```{r undersupported_genes, fig.height=23, fig.width=8, echo = FALSE, fig.cap = undersupported_genes_cap}
