...
```{r consist_genes_heatmap, fig.height=20, fig.width=10, fig.cap=consist_genes_heatmap_cap}
...
## Tease apart the closely neurogenic and RPC cell types

```{r neuro, fig.width=25, fig.height=8, echo = F, message = F, fig.cap=neuro_cap}
...
lu_rep_plot <- lu_long %>% 
  ...
  coord_flip() + 
  cowplot::theme_cowplot()  + geom_text(aes(y=7), position = position_dodge2()) + facet_wrap(~Organism)
...
## Pan-study fovea vs peripheral cone test identifies 16 genes consistently differentially expressed

While we have provided data in many forms via the web app at plae.nei.nih.gov, there are many more dissections of the data that can be performed. To facilitate custom testing of scEiaD we provide as an example a human fovea versus peripheral cone gene differential testing document as supplementary file "pseudobulk_cone_region.freeze01.html". Yan et al. proposed PRPH2 and RS1 as markers to distinguish fovea and peripheral located cones [@yan_cell_2020]. In our analysis document we use the Seurat object containing the full scEiaD dataset we provide at plae.nei.nih.gov -> Data to build a custom pseudobulk counts matrix containing only cones from human studies that were taken in a region specific manner. This left us with 


To see whether we could replicate these findings and whether we can propose new markers we performed a custom differential test in human cones that were derived from studies that took retina punches from the fovea or peripheral regions. As this custom test was not 
...
```{r neuro_table, echo = F, message = F, fig.cap=neuro_table_cap}
