...
scEiaD_2020_v01 <- pool::dbPool(RSQLite::SQLite(), dbname =  glue('{data_dir_vPLAE}/MOARTABLES__anthology_limmaFALSE___4000-counts-universe-study_accession-scANVIprojection-15-5-0.1-50-20.sqlite'))
...
nun3 <- meta_filter %>% filter(Organ == 'Eye') %>% pull(CellType) %>% unique() %>% length()
...
```{r}
scEiaD_2020_v01 %>% tbl('wilcox_diff_testing') %>% 
  filter(Group == 'CellType (Predict)', FDR < 1e-5, mean_auc > 0.7 ) %>% 
  filter(Base %in% (meta_filter %>% filter(Organ == 'Eye') %>% pull(CellType_predict) %>% unique())) %>% 
  group_by(Base) %>% 
  summarise(Count = n()) %>% 
