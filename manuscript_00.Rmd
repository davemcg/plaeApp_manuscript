...
x <- scEiaD_2020_v01 %>% 
  ...
  collect() %>% 
  filter(Group == 'CellType (Predict)', 
         Organism == org, Against == 'All', Base %in% ct) %>% 
  select(Gene, log2FoldChange, Base) %>% 
  pivot_wider(values_from=log2FoldChange, names_from = Base) 
celltype <- colnames(x[,-1])
x <- x %>% data.frame()
