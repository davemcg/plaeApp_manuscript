...
```{r}
scEiaD_2020_v01 %>% tbl('wilcox_diff_testing') %>% filter(Group == 'CellType (Predict)', FDR < 1e-5, mean_auc > 0.7 ) %>% 
  group_by(Base) %>% 
  summarise(Count = n()) %>% 
