...
spec_diff <- diff_tab_big %>% 
    filter(Gene %in% diff_tab$Gene) %>% # only retain diff genes across "All"
    group_by(Gene, Organism, Base) %>% 
    summarise(Count = n()) %>% 
    filter(Count > 1) %>% summarise(Count = n()) %>% filter(Count < 3) %>% mutate(go = paste(Gene, '___', Organism, sep = '')) %>% pull(go)

diff_tab_big %>% 
    filter(Gene %in% diff_tab$Gene) %>% # only retain diff genes across "All"
    group_by(Gene, Organism, Base) %>% 
    summarise(Count = n()) %>% 
    filter(Count > 1) %>% mutate(go = paste(Gene, '___', Organism, sep = '')) %>% filter(go %in% (spec_diff))  %>% select(-go) %>% ungroup() %>% group_by(Organism, Base) %>% slice_max(n = 10, order_by = Count) %>% filter(Base == 'Rod')
```
...
consistent <- diff_tab %>% 
  ...
  summarise(Count = n()) %>% 
  filter(Count > 1) %>% 
  mutate(k = paste(Gene, Base, sep = '_'))

diff_tab %>% mutate(k = paste(Gene, Base, sep = '_')) %>% 
...
down_tab <- scEiaD_2020_v01 %>% tbl('diff_testing') %>% 
  filter(Group == 'CellType (Predict)', padj < 1e-5, (log2FoldChange) < -2) %>% 
  collect() %>% 
  filter(Against == 'All', Base %in% (meta_filter %>% filter(Organ == 'Eye') %>% pull(CellType_predict) %>% unique())) %>% mutate(k = paste(Gene, Base, sep = '_'))
