...
sup_tab_consist_diff <- consist_diff %>% filter(Symbol %in% uniq_g$Symbol) %>% group_by(Base) %>% summarise(Count = n()) %>% ggplot(aes(x=`Base`, y = `Count` )) + geom_bar(stat='identity', position = 'dodge') + cowplot::theme_cowplot() + coord_flip() + xlab("CellType (Predict)")
#toc()
```
...
# # arrange(-`mean log2FC`)
# 
down <- pairwise_neurogenic_test %>% filter(`sig count` > 1) %>% arrange(`mean padj`) %>% filter(`mean log2FC` < 0, `mean padj` < 0.05) %>% head(5) %>% pull(Gene)
up <-  pairwise_neurogenic_test %>% filter(`sig count` > 1) %>% arrange(`mean padj`) %>% filter(`mean log2FC` > 0) %>% head(9) %>% pull(Gene)
...
neuro_plot <- plot_grid(plot_grid(exp_plot_down + theme(legend.position="none") + xlab(''),
          exp_plot_up + theme(legend.position="none") + ylab('') + xlab(''), labels = c('b','c'),
...
cowplot::plot_grid(lu_rep_plot, neuro_plot, rel_widths = c(0.7,1), labels = c('a'))
```
...
pairwise_neurogenic_test_ungrouped %>% filter(abs(log2FoldChange) > 2, padj < 0.05) %>% flextable() %>% autofit() %>% fit_to_width(7)
```
