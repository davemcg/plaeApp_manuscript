...
scEiaD_data_overview_cap <- fig_cap(name = 'scEiaD_data_overview', caption = glue::glue("Distribution of tissues and cell types in scEiaD. A. Number of cells for each ocular cell type. B. Number of studies for each ocular cell type (over 10 cells), colored by species. C. Tabulation of the number of studies present across {num1} ocular and {num2} non-ocular tissues, D. Web app performance (lower is better, missing functionality is shown as blank) for important functions compared between PLAE, Spectacle, and Tabula Sapiens (see methods)."))
...
ab <- cowplot::plot_grid(cell_counts + theme(legend.position = "none"), 
                         study_counts_by_cell, 
                         nrow = 1, 
                         rel_widths = c(1,1), labels = c('a','b'))

cd <- cowplot::plot_grid(study_count_by_tissue, 
                         timing_plot, 
                         nrow = 1, 
                         rel_widths = c(0.7,1),
                         labels = c('c','d'))
